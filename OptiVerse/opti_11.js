...
app.post('/post-thought', (req, res) => {
    ...
    req.on("end", () => {
        ...
        const query = "INSERT INTO userposts (username, post, date) VALUES (?, ?, FROM_UNIXTIME(?))";
        const values = [req.session.usermail, data.thought, date]; // replace 'req.session.username' with the actual username
        pool.getConnection((err, connection) => {
            if (err) throw err;
            connection.query(query, values, (err, result) => {
                connection.release();
